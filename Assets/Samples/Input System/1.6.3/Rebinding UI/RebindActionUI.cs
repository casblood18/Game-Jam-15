...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            if (m_RebindText != null)
            {
                var text = !string.IsNullOrEmpty(m_RebindOperation.expectedControlType)
                    ? $"Waiting for < {actionName} > input. {partName}"
                    : $"{partName}Waiting for < {actionName} > input.";
        ...
        private bool CheckDuplicateBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            // Loop over all action maps in the asset
            foreach (var actionMap in asset.actionMaps)
            {
                // Loop over all bindings in the action map
                foreach (InputBinding binding in actionMap.bindings)
                {
                    // Check if the action of the binding is the same as the action of the new binding
